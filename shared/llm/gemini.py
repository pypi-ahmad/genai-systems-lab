...
class GeminiTimeoutError(GeminiGenerationError):
    """Raised when a Gemini request exceeds the timeout."""
...
def _get_client() -> genai.Client:
    """Return a cached Gemini client for the current per-request API key."""
...
def _request_content_single(
    *, prompt: str, model: str, config: types.GenerateContentConfig | None = None
    ...
    """Try *model* with retries + exponential backoff and timeout enforcement."""
    ...
    for attempt in range(1, MAX_RETRY_ATTEMPTS + 1):
        try:
            ...
            LOGGER.warning(
                "Gemini request timed out: model=%s attempt=%s",
                model, attempt,
            )
            raise
            ...
            raise GeminiGenerationError("Unexpected Gemini request failure.") from exc
...
def _request_content(
    *, prompt: str, model: str, config: types.GenerateContentConfig | None = None
    ...
    """Try the requested model, falling back to a cheaper model on failure."""
    try:
        return _request_content_single(prompt=prompt, model=model, config=config)
    except (GeminiGenerationError, GeminiTimeoutError) as primary_err:
        fallback = MODEL_FALLBACK.get(model)
        if not fallback or fallback == model:
            raise
        LOGGER.warning(
            "Falling back from %s to %s after error: %s",
            model, fallback, primary_err,
        )
...
def generate_text(prompt: str, model: str) -> str:
    ...
    if not text:
        raise GeminiGenerationError("Gemini returned an empty text response.")
    return text
...
def generate_structured(prompt: str, model: str, schema: dict[str, Any]) -> dict[str, Any]:
    ...
    raise GeminiGenerationError("Gemini returned an invalid structured response.")


def _vision_request_single(
    *, prompt: str, image: bytes, model: str,
) -> types.GenerateContentResponse:
    """Vision call with retries, backoff, and timeout enforcement."""
    _validate_model(model)
    ...
    for attempt in range(1, MAX_RETRY_ATTEMPTS + 1):
        try:
            ...
            LOGGER.warning(
                "Gemini vision request timed out: model=%s attempt=%s",
                model, attempt,
            )
            raise
        except (errors.APIError, ValueError) as exc:
            ...
            raise GeminiGenerationError("Unexpected Gemini vision failure.") from exc
...
def generate_text_from_image(prompt: str, image: bytes, model: str) -> str:
    try:
        ...
        if not fallback or fallback == model:
            raise
        LOGGER.warning(
            "Vision falling back from %s to %s after error: %s",
            model, fallback, primary_err,
        )
    ...
    text = (response.text or "").strip()
    if not text:
        raise GeminiGenerationError("Gemini returned an empty vision response.")
    return text