...
def call_api(prompt: str, options: dict, context: dict) -> dict:
    ...
    if not query:
        return {"error": "Promptfoo rendered an empty RAG query."}
    ...
    try:
        ...
        with request_overrides(config):
            ...
            retrieved_chunks = _normalize_chunks(module.retrieve(query))
            context_text = module.build_context(retrieved_chunks)
            answer = module.answer_question(query, context_text)
        ...
        return {
            "output": {
                ...
                "retrieved_files": [chunk["path"] for chunk in retrieved_chunks],
